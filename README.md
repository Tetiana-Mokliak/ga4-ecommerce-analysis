## ga4-ecommerce-analysis
SQL analysis of GA4 e-commerce data in BigQuery: product funnel, ABC segmentation, basket analysis, and user behavior insights.
## Technologies
- SQL
- BigQuery
- Google Analytics 4 sample ecommerce dataset
- Google Sheets

## SQL techniques used
- CTE
- Window Functions
- UNNEST
- CASE WHEN
- SAFE_DIVIDE
- Self Join
- Aggregations

Метою проєкту було дослідити аналіз товарного асортименту та купівельної поведінки користувачів на основі GA4 e-commerce датасету (BigQuery). Включає ABC-аналіз, basket analysis та продуктову воронку.

## 1. ABC-аналіз товарного асортименту

У цьому блоці проведено аналіз продажів на основі GA4 e-commerce dataset.

Мета аналізу — визначити товари, які формують основну частину виручки, та розподілити їх за ABC-категоріями.

Для аналізу використано:
- таблиці `ga4_obfuscated_sample_ecommerce.events_*` у BigQuery;
- події `purchase`;
- масив `items`, розгорнутий за допомогою `UNNEST(items)`.

Основні кроки:
- розрахунок виручки по кожному товару;
- ранжування товарів за виручкою;
- розрахунок частки товару в загальній виручці;
- розрахунок накопичувальної частки виручки;
- присвоєння ABC-категорії.

Для класифікації використано принцип Парето: товари категорії A формують основну частину виручки (~80%).
![ABC Analysis](abc_analysis.png)

## Висновки
- Близько 27% товарів формують ~79% загальної виручки, що відповідає принципу Парето.
- Найбільший внесок у виручку забезпечують товари категорії `Apparel`, що свідчить про високу концентрацію попиту в цьому сегменті.
- Значна частина асортименту (група C) має мінімальний внесок у дохід (~5%), що може вказувати на потенціал для оптимізації асортименту або перегляду стратегії продажів для цих товарів.

## 2. Basket Analysis

Мета аналізу — виявити найпоширеніші комбінації товарів у межах одного замовлення.
Для аналізу використано той самий GA4 e-commerce dataset.

Основні кроки:
- вибрано дані за січень 2021 року;
- використано події `purchase`;
- масив товарів розгорнуто за допомогою `UNNEST(items)`;
- сформовано пари товарів через `self join` таблиці;
- підраховано кількість спільних покупок для кожної пари товарів;
- для аналізу залишено лише комбінації, що зустрічаються більше 5 разів.
  
![Basket Analysis](Basket.png)

## Висновки
Отримані пари товарів можуть бути використані для перехресних рекомендацій (cross-sell).

## 3. Product Funnel Analysis
## Мета аналізу

Проаналізувати поведінку користувачів на рівні товарів у воронці перегляд → додавання в кошик → покупка`та визначити:
- які товари мають високу конверсію в покупку;
- які товари привертають увагу, але не конвертуються у продаж;
- на якому етапі воронки користувачі найчастіше припиняють взаємодію.

Для аналізу використано той самий GA4 e-commerce dataset.

Основні кроки:
- масив товарів розгорнуто за допомогою `UNNEST(items)`;
- вибрано події:
  - `view_item`,
  - `add_to_cart`,
  - `purchase`;
- для кожного товару створено маркери наявності події;
- підраховано кількість користувачів на кожному етапі воронки;
- розраховано конверсії:
  - `add_to_cart / view_item`,
  - `purchase / view_item`,
  - `purchase / add_to_cart`;
- товари сегментовано за поведінковими характеристиками та рівнем конверсії.

  ![Product Funnel Analysis](Product_Funnel_Analysis.png)

## Висновки

- Найбільша частка товарів належить до категорії `checkout_issue`, що може свідчити про втрати користувачів на етапі оформлення покупки.
- Категорія `high_potential` містить товари з високим інтересом, але низькою конверсією в покупку.
- Товари категорії `low_visibility` мають хорошу конверсію при низькій кількості переглядів, що вказує на потенціал при збільшенні видимості.
- Виявлено товари з високим рівнем переглядів, але без покупок (`critical_issue`), що може свідчити про проблеми з товаром або трекінгом.
